...
const validateAccordionWidget = accordionWidget => {
  ...
  if (accordionItems.length === 0) {
    console.warn(
      "Document Error: no accordion items found inside the accordion widget."
    )
    return
  }
  ...
  accordionItems.forEach((item, index) => {
    const heading = item.querySelector(":scope > h3,:scope >  h4")
    const content = item.querySelector(".accordion-content")

    // If there are no heading for the accordion item, log a warning
    if (!heading) {
      console.warn(
        `Document Error: no heading found for accordion item ${index + 1}.`
      )
      return
    }
