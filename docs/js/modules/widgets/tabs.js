  ...
  const callTabsWidget = () => {
    ...
    tabsWidgetsGroups.forEach((tabsWidgetsGroup, tabsWidgetsGroupIndex) => {
      ...
      if (tabItems.length > 0) {
        // Add region and aria-label to parent div if not already set
        if (!tabsWidgetsGroup.hasAttribute("role")) {
          tabsWidgetsGroup.setAttribute("role", "region");
        }
        if (!tabsWidgetsGroup.hasAttribute("aria-label")) {
          tabsWidgetsGroup.setAttribute("aria-label", `tab group ${tabGroupNum}`);
        }

        tabItems.forEach((tabItem, tabItemIndex) => {
          // Check for existing tab input
          if (!tabItem.querySelector(".tab-input")) {
            // Create tab input
          ...
          // Check for existing tab label
          if (!tabItem.querySelector(".tab-header")) {
          ...
          // Check for existing tab-content attributes
          const tabContent = tabItem.querySelector(".tab-content");
          if (tabContent) {
            if (!tabContent.classList.contains("tab-panel")) {
            ...
            if (tabItemIndex !== tabItems.length - 1) {
              // Error check: if tab-content tag is missing and it's not the last item
              console.warn(
          ...
          if (tabItemIndex + 1 === tabItems.length) {
            ...
            if (hideTabCheck) {
              if (hideTabCheck.textContent !== "Hide") {
                ...
                if (!tabsWidgetsGroup.querySelector(".hide-tab")) {
                  const hideTabItem = document.createElement("div");
                  ...
                  const hideTabTitle = document.createElement("span");
          ...
          // Add attributes to ensure the first tab is open
          if (tabItemsPerGroup === 0) {
            const tabInput = tabItem.querySelector(".tab-input");
            if (tabInput) {
